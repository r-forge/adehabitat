...
\texttt{ltraj}. For example, load the dataset \texttt{capreochiz}:
...
bridge kernel method (Bullard 1999, Horne et al. 2007) and the product
kernel algorithm (Keating and Cherry 2009) are implemented in the functions
\texttt{kernelbb} and \texttt{kernelkc} respectively.\\
